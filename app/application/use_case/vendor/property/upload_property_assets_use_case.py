...
class VendorUploadPropertyAssetsUseCase(PropertySerializerMixin, BaseUseCase):
    FILE_UPLOAD_RULES = {
        "files": {
    ...
    async def execute(
        ...
        # Ownership check — vendor can only upload media to their own properties
        if property_.vendor_id != self.current_user.id:
            raise AppException(
                status_code=403,
                message="You are not authorized to upload media to this property.",
                field="property_id",
        ...
        # Replace existing FEATURE / COVER assets when new ones are provided
...
class VendorDeletePropertyAssetUseCase(PropertySerializerMixin, BaseUseCase):
    def __init__(
        self,
        property_service: PropertyService,
        property_asset_service: PropertyAssetService,
    ...
    async def execute(self, property_id: str, asset_id: str) -> Property:
        ...
        # Ownership check — vendor can only delete assets from their own properties
        if property_.vendor_id != self.current_user.id:
            raise AppException(
                status_code=403,
                message="You are not authorized to delete assets from this property.",
                field="property_id",
                error_code="PROPERTY_ACCESS_DENIED",
            )

...
class VendorGetPropertyUseCase(PropertySerializerMixin, BaseUseCase):
    def __init__(
        self,
    ...
    async def execute(self, property_id: str) -> Optional[dict]:
        ...
        # Ownership check — vendor can only view their own properties
        if property_data.vendor_id != self.current_user.id:
            raise AppException(
                status_code=403,

...
class VendorCreatePropertyUseCase(PropertySerializerMixin, BaseUseCase):
    def __init__(
        self,
    ...
    async def execute(self, property_dto: PropertyDTO) -> Optional[dict]:
        ...
        duplicate_name = await self.property_service.get_by_vendor_and_name(
            vendor_id, property_dto.name, flush=True
        )
        if duplicate_name:
            raise AppException(
                status_code=409,
        ...
        property_dto.slug = await self._generate_unique_slug(vendor_id, base_slug)
    ...
    async def _generate_unique_slug(self, vendor_id: int, base_slug: str) -> str:
        """

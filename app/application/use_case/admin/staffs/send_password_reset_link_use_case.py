...
from app.application.dto.staffs.staff import StaffResetLinkDTO
...
class SendStaffPasswordResetLinkUseCase:
    def __init__(
        self,
    ...
    async def execute(self, staff_id: str, data: StaffResetLinkDTO):
        ...
        user = await self.user_service.get_user_by_public_id(staff_id)
        ...
        if user.status != UserStatus.ACTIVE:
            raise AppException(
                status_code=status.HTTP_400_BAD_REQUEST,
                message="Staff account is not active. Password reset link can only be sent to active accounts.",
        ...
        for existing_token in existing_tokens:
            ...
            if expires_at < datetime.now(timezone.utc):
                ...
                logger.info("Revoked expired password reset token for staff user %s", user.id)
            else:
                raise AppException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            logger.warning("Failed to publish ForgotPasswordEvent for staff %s: %s", user.id, exc)
